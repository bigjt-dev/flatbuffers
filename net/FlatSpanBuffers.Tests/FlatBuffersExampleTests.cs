...
namespace FlatSpanBuffers.Tests
{
    ...
    public class FlatBuffersExampleTests
    {
        ...
        public void TestFixedLenghtArrays()
        {
            ...
            float a;
            Span<int> b = stackalloc int[15];
            sbyte c;
            Span<int> d_a = stackalloc int[4]; // flattened int[2, 2];
            Span<TestEnum> d_b = stackalloc TestEnum[2];
            Span<TestEnum> d_c = stackalloc TestEnum[4]; // flattened TestEnum[2, 2];
            Span<long> d_d = stackalloc long[4]; // flattened long[2, 2];
            int e;
            Span<long> f = stackalloc long[2];
            ...
            d_b[0] = TestEnum.B;
            d_b[1] = TestEnum.C;
            d_c[0 * 2 + 0] = TestEnum.A;
            d_c[0 * 2 + 1] = TestEnum.B;
            d_c[1 * 2 + 0] = TestEnum.C;
            d_c[1 * 2 + 1] = TestEnum.B;
            d_d[0 * 2 + 0] = -1;
            d_d[0 * 2 + 1] = 1;
            d_d[1 * 2 + 0] = -2;
            d_d[1 * 2 + 1] = 2;
